...
const Page = () => {
  ...
  const [minDischargeDate, setDischargeMinDate] = useState(null)
  ...
  const handleChange = (e) => {
    ...
    if (name === "amount") {
      ...
      if (selectedBenefit?.AMOUNT_LIMIT && newValue > selectedBenefit.AMOUNT_LIMIT) {
  ...
  const FileInput = ({
    ...
    return (
      <div className="border border-white/20 rounded-xl p-4 bg-gray-900/40 hover:bg-gray-900/60 transition">
        ...
        <ul className="mt-2">
          {files.map((file) => (
            <li key={file.name} className="flex justify-between items-center mt-1 bg-gray-800 p-2 rounded">
              <span>{file.name}</span>
              <button
                type="button"
  ...
  useEffect(() => {
    if (formData.claimType) {
      ...
      setDischargeMinDate(formData.claimType === "103" ? null : minAllowedDate)
    }
  }, [formData.claimType, selectedSlot])
