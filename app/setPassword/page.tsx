...
type FuturisticResetFormProps = {
    onSubmit: (e: FormEvent<HTMLFormElement>) => void;
}

export default function FuturisticResetForm({ onSubmit }: FuturisticResetFormProps) {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        // await fetch('/api/auth/reset-password', { method: 'POST', body: JSON.stringify({ userid, password }) })
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0f172a] via-[#07103a] to-[#02203a] p-6">
            <form
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-300">Enter your User ID or mobile and pick a secure password.</p>
                    </div>
                </div>
                ...
                <button type="button" onClick={()=> router.replace('/login') } className="text-sm text-gray-400 hover:underline cursor-pointer">Back</button>
            </form>
        </div>
    );
}
